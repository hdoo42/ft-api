...
use crate::{AccessToken, AuthInfo, FtLocations};
...
struct FtApiClientHttpSessionApi<FCHC>
where
    FCHC: FtClientHttpConnector + Send,
{
    token: AccessToken,
...
impl FtApiClientHttpSessionApi {
    async fn new() -> Self {
    ...
    async fn campus_gs_locations(&self) -> Vec<FtLocations> {
        self.client.get("")
    }
}

pub trait FtClientHttpConnector {
    fn http_get_uri<'a, RS>(&'a self, full_uri: Url) -> BoxFuture<'a, ClientResult<RS>>
    where
        ...
        TS: AsRef<str> + 'p + Send,
    {
        ...
        match full_uri {
            Ok(full_uri) => self.http_get_uri(full_uri),
        ...
        RS: for<'de> serde::de::Deserialize<'de> + Send + 'a,
    {
        match self.create_method_uri_path(method_relative_uri) {
            Ok(full_uri) => self.http_post_uri(full_uri, request),
            Err(err) => std::future::ready(Err(err)).boxed(),
        }
    }
